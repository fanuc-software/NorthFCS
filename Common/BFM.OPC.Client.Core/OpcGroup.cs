...
namespace BFM.OPC.Client.Core
{
	public class OpcGroup
	{
	    ...
	    public List<string> m_itemIds = new List<string>();
		...
		public ItemResult[] AddItems(string[] itemIds, Guid[] ClientHandles = null)
		{
			...
			if (count <= 0) return new ItemResult[0];
			...
			out int cancelID)
		{
			try
			{
				...
				// unmarshal output parameters.
				int[] errors = HD.OPC.Client.Core.Com.Interop.GetInt32s(ref pErrors, itemIDs.Length, true);
			...
			{
				throw HD.OPC.Client.Core.Com.Interop.CreateException("IOPCAsyncIO2.Read", e);
			}
		}
        ...
        public void Refresh()
		{
			lock (this)
			{
				...
				{
					throw HD.OPC.Client.Core.Com.Interop.CreateException("IOPCAsyncIO2.Refresh", e);
				}
			}
		}
		...
		{
			add { lock (this) { m_callback.DataChanged -= value; m_callback.DataChanged += value; Advise(); } }
		...
		private OpcRcw.Da.OPCITEMDEF[] GetOPCITEMDEFs(OpcItem[] input)
		{
			...
			if (input != null)
			{
				...
				for (int ii = 0; ii < input.Length; ii++)
				{
					...
					output[ii].szAccessPath = input[ii].ItemPath ?? String.Empty;
					output[ii].bActive = 1;
					//todo 
					//output[ii].vtRequestedDataType = 2;
					output[ii].vtRequestedDataType = (short)VarEnum.VT_EMPTY;//(short)HD.OPC.Client.Core.Com.Interop.GetType(input[ii].ReqType);
					//output[ii].vtRequestedDataType = 4;
