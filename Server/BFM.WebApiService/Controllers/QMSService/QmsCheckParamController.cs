...
{
    /// <summary>
    /// QmsCheckParam ApiController
    /// </summary>
    [AllowAnonymous]
    [RoutePrefix("QMSService/QmsCheckParam")]
    public class QmsCheckParamController : ApiController 
    {
        #region QmsCheckParam分页查询
        ...
        [Route("GetPageData")]
        [HttpGet]
        public List<QmsCheckParam> GetQmsCheckParamByPage(int pageSize, int pageIndex, bool isAsc, string orderField, string sWhere)
        {
            string orderStr = "";
            ...
            {
                List<QmsCheckParam> models = QmsCheckParamBLL.GetModelsByPage(pageSize, pageIndex, isAsc, orderStr, whereLamda);
                return models;
            }
        }
        ...
        [Route("Add")]
        [HttpPost]
        public bool AddQmsCheckParam(QmsCheckParam mQmsCheckParam)
        {
            if (mQmsCheckParam == null) return false;
            using (IQmsCheckParamBLL QmsCheckParamBLL = BLLContainer.Resolve<IQmsCheckParamBLL>())
            {
                return QmsCheckParamBLL.Add(mQmsCheckParam);
            }
        }
        ...
        [Route("GetByParam")]
        [HttpPost]
        public List<QmsCheckParam> GetQmsCheckParams([FromBody]string sWhere)
        {
            Expression<Func<QmsCheckParam, bool>> whereLamda = SerializerHelper.ConvertParamWhereToLinq<QmsCheckParam>(sWhere);
            using (IQmsCheckParamBLL QmsCheckParamBLL = BLLContainer.Resolve<IQmsCheckParamBLL>())
            {
                List<QmsCheckParam> models = QmsCheckParamBLL.GetModels(whereLamda);
                return models;
            }
        }
