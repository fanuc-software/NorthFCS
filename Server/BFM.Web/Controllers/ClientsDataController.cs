...
using BFM.WCFService;
using BFM.Web.Base.Operator;

namespace BFM.Web.Controllers
{
    [HandlerLogin]
    public class ClientsDataController : Controller
    {
        ...
        private object GetMenuList()
        {
            ...
            return ToMenuJson(new SDMService().GetSysMenuItems("TARGET_NAME = 'Web'"), "0");
        }
        private string ToMenuJson(List<SysMenuItem> data, string parentId)
        {
            StringBuilder sbJson = new StringBuilder();
            sbJson.Append("[");
            List<SysMenuItem> entitys = data.FindAll(t => t.PARENT_PKNO == parentId);
            if (entitys.Count > 0)
            {
                foreach (var item in entitys)
                {
                    string strJson = item.ToJson();
                    strJson = strJson.Insert(strJson.Length - 1, ",\"ChildNodes\":" + ToMenuJson(data, item.PKNO) + "");
