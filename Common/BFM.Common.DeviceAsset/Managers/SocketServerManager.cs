...
namespace BFM.Common.DeviceAsset
{
    ...
    /// </summary>
    public class SocketServerManager : IDeviceCore
    {
        ...
        private List<string> CurClientResult = new List<string>();  //Client当前发送的值
        ...
        private ISocketCommDevice SockeDevice = null;  //专用Socket接口

        private List<string> UnFinishWriteData = new List<string>(); //未完成写入的数据 

        #endregion

        public SocketServerManager(DeviceManager devcieCommunication, DataChangeEventHandler callback)
        {
            ...
            List<DeviceTagParam> deviceTagParams, DataChangeEventHandler callback)
        {
            ...
            ThreadPool.QueueUserWorkItem(s => { CreateAndRefreshServer(); }); //不断重连
        ...
        /// 创建和刷新Client
        /// </summary>
        private void CreateAndRefreshServer()
        {
            ...
            while(!CBaseData.AppClosing && !bCloseSocket)
            {
                ...
                lock (lockcreate)
                {
                    ...
                    {
                        _server?.Dispose();
                        _server.OnReadClientBytes -= ServerOnOnReadClientBytes; //获取到客户端的消息
                        _server = null;
                    }

                    if (ServerIP == null)
                    {
                        _server = new SocketServer(ServerPort, MaxClients);
                    }
                    else
                    {
                        _server = new SocketServer(ServerIP, ServerPort, MaxClients);
                    }
                    ...
                    if (_server.IsRunning) //服务端建立成功
                    {
                        #region 发送缓存中的内容

                        for (int i = 0; i < UnFinishWriteData.Count; i++)
                        {
                            int result = _server.Send(UnFinishWriteData[i]);
                            if (result != 0)
                            {
                                break;
                            }
        ...
        private void ServerOnOnReadClientBytes(object sender, byte[] bytes)
        {
            ...
            lock (lockanysis)
            {
                ...
                if (SockeDevice == null)
                {
                    ...
                    CurClientResult.Clear();
                    CurClientResult.Add(str);
                ...
                {
                    CurClientResult = SockeDevice?.AnalysisServerData(bytes, out error); //解析数据
                }
                ...
                if (error != "")
                {
                    EventLogger.Log($"已接收到客户端数据，解析错误：'{error}'");
                    Thread.Sleep(100);
                    return;
                }
                ...
                if (CurClientResult.Count >= 2)
                {
                    string dataAddress = CurClientResult[0];
                    string dataValue = CurClientResult[1];
        ...
        public OperateResult SyncWriteData(string dataAddress, string dataValues)
        {
            ...
            if (_server == null)
            {
                return new OperateResult("TCP 服务端 未初始化，请先初始化.");
            }

            if (!_server.IsRunning)
            {
                if (!bCloseSocket) _server.Start();

                if (!_server.IsRunning) return new OperateResult("自TCP 服务端 未运行，请重试.");
            }
            ...
            lock (lockwrite)
            {
                ...
                int result = _server.Send(sendData);  //向设备写入数据
        ...
        public OperateResult<string> SyncReadData(string dataAddress)
        {
            ...
            if (_server == null)
            {
                return new OperateResult<string>("TCP 服务端 未初始化，请先初始化.");
            }
            ...
            if (!_server.IsRunning)
            {
                ...
                if (!_server.IsRunning) return new OperateResult<string>("自TCP 服务端 未运行，请重试.");
            }
            ...
            lock (lockread)
            {
                ...
                int result = _server.Send(sendData);  //向Socket发送读取指令
        ...
        public OperateResult AsyncWriteData(string dataAddress, string dataValues)
        {
            ...
            lock (lockwrite)
            {
                ...
                int result = _server.AsyncSend(sendData);  //向Socket发送读取指令
