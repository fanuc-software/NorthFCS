...
{
    /// <summary>
    /// MesProcessCtrol ApiController
    /// </summary>
    [AllowAnonymous]
    [RoutePrefix("PLMService/MesProcessCtrol")]
    public class MesProcessCtrolController : ApiController 
    {
        ...
        [Route("GetByParam")]
        [HttpPost]
        public List<MesProcessCtrol> GetMesProcessCtrols([FromBody]string sWhere)
        {
            Expression<Func<MesProcessCtrol, bool>> whereLamda = SerializerHelper.ConvertParamWhereToLinq<MesProcessCtrol>(sWhere);
            using (IMesProcessCtrolBLL MesProcessCtrolBLL = BLLContainer.Resolve<IMesProcessCtrolBLL>())
            {
