...
{
    /// <summary>
    /// RsWorkShop ApiController
    /// </summary>
    [AllowAnonymous]
    [RoutePrefix("RSMService/RsWorkShop")]
    public class RsWorkShopController : ApiController 
    {
        ...
        [Route("GetByParam")]
        [HttpPost]
        public List<RsWorkShop> GetRsWorkShops([FromBody]string sWhere)
        {
            Expression<Func<RsWorkShop, bool>> whereLamda = SerializerHelper.ConvertParamWhereToLinq<RsWorkShop>(sWhere);
            using (IRsWorkShopBLL RsWorkShopBLL = BLLContainer.Resolve<IRsWorkShopBLL>())
            {
