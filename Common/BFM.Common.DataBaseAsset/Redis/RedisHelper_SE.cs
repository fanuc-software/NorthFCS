...
namespace BFM.Common.DataBaseAsset.Redis
{
    /// <summary>
    /// Redis 助手
    /// </summary>
    public class RedisHelper_SE
    {
        /// <summary>
        /// 连接字符串
        /// </summary>
        ...
        static RedisHelper_SE()
        {
            ConnectionString = "127.0.0.1:6379"; //ConfigurationManager.ConnectionStrings["RedisConnectionString"].ConnectionString;
            _connMultiplexer = ConnectionMultiplexer.Connect(ConnectionString);
            DefaultKey = "Get"; //ConfigurationManager.AppSettings["Redis.DefaultKey"];
            AddRegisterEvent();
        }

        public RedisHelper_SE(int db = -1)
        {
