...
namespace HD.OPC.Client.Core.Com
{
	/// <summary>
	...
	public class Interop
	{
		...
		{
			get { lock (typeof(HD.OPC.Client.Core.Com.Interop)) { return m_preserveUTC; } }
			set { lock (typeof(HD.OPC.Client.Core.Com.Interop)) { m_preserveUTC = value; } }
		}
		...
		public static IntPtr GetFILETIMEs(DateTime[] datetimes)
		{
			...
			{
				Marshal.StructureToPtr(HD.OPC.Client.Core.Com.Interop.GetFILETIME(datetimes[ii]), pos, false);
		...
		public static DateTime[] GetFILETIMEs(ref IntPtr pArray, int size, bool deallocate)
		{
			...
			for (int ii = 0; ii < size; ii++)
			{
				datetimes[ii] = HD.OPC.Client.Core.Com.Interop.GetFILETIME(pos);
				pos = (IntPtr)(pos.ToInt32() + Marshal.SizeOf(typeof(FILETIME)));
			}
		...
		public static IntPtr GetVARIANTs(object[] values, bool preprocess)
		{
			...
			IntPtr pValues = Marshal.AllocCoTaskMem(count * HD.OPC.Client.Core.Com.Interop.VARIANT_SIZE);
			...
			for (int ii = 0; ii < count; ii++)
			{
				if (preprocess)
				{
					Marshal.GetNativeVariantForObject(HD.OPC.Client.Core.Com.Interop.GetVARIANT(values[ii]), pos);
				}
				else
				{
				...
				pos = (IntPtr)(pos.ToInt32() + HD.OPC.Client.Core.Com.Interop.VARIANT_SIZE);
			}
		...
		public static object[] GetVARIANTs(ref IntPtr pArray, int size, bool deallocate)
		{
			...
			for (int ii = 0; ii < size; ii++)
			{
				try
				{
					...
					if (deallocate) HD.OPC.Client.Core.Com.Interop.VariantClear(pos);
				}
				catch (Exception)
				{
					values[ii] = null;
		...
		internal static string GetLocale(int input)
		{
			...
			{
				throw new ExternalException("Invalid LCID", HD.OPC.Client.Core.Com.ResultIDs.E_INVALIDARG);
			}
		}
		...
		internal static System.Type GetType(VarEnum input)
		{
			switch (input)
			{
				...
				default: return HD.OPC.Client.Core.Com.Type.ILLEGAL_TYPE;
			}
		}
		...
		internal static VarEnum GetType(System.Type input)
		{
			...
			if (input == HD.OPC.Client.Core.Com.Type.ILLEGAL_TYPE) return (VarEnum)Enum.ToObject(typeof(VarEnum), 0x7FFF);
			if (input == typeof(System.Type)) return VarEnum.VT_I2;
			if (input == typeof(HD.OPC.Client.Core.Com.Quality)) return VarEnum.VT_I2;
			if (input == typeof(HD.OPC.Client.Core.Com.accessRights)) return VarEnum.VT_I4;
			if (input == typeof(HD.OPC.Client.Core.Com.euType)) return VarEnum.VT_I4;
		...
		{
			return new HD.OPC.Client.Core.Com.ResultIDException(HD.OPC.Client.Core.Com.Interop.GetResultID(code), message);
		}
